...
export function FileUploadInput({
        ...
        img.onload = () => {
            ...
            if (exactDimensions) {
                if (img.width !== exactDimensions.width || img.height !== exactDimensions.height) {
                    reject(new Error(`Image must be exactly ${exactDimensions.width}x${exactDimensions.height} pixels.`));
                    return;
                }
            }
            ...
            if (aspectRatio) {
                ...
                if (Math.abs(ratio - aspectRatio) > tolerance) {
                    ...
                    const expected = aspectRatio === 1 ? "1:1" : aspectRatio === 16/9 ? "16:9" : `${aspectRatio}:1`;
                    reject(new Error(`Image aspect ratio must be ${expected}.`));
                    return;
                }
            }
  ...
  const handleFileSelect = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 1. Get Presigned URL
      const presignResult = await getPresignedUrl(type, file.type, file.size);
      
      if (!presignResult.success || !presignResult.data) {
        throw new Error(presignResult.error || "Failed to initiate upload");
      }
  ...
  const validationText = [
      aspectRatio ? `Ratio: ${aspectRatio === 1 ? "1:1" : aspectRatio === 16/9 ? "16:9" : aspectRatio}` : "",
      exactDimensions ? `${exactDimensions.width}x${exactDimensions.height}px` : "",
    ...
    <div className={cn("space-y-2", className)}>
      ...
      <div 
        className={cn(
            ...
            (type === "banner" || type === "channel-banner") ? "h-40 w-full aspect-video" : "h-32 w-32",
      ...
      >
        {isUploading ? (
            ...
            <div className="flex flex-col items-center gap-2 text-muted-foreground text-center">
                {String(type).includes("avatar") || String(type).includes("logo") ? <ImageIcon className="h-8 w-8 opacity-50" /> : <Upload className="h-8 w-8 opacity-50" />}
