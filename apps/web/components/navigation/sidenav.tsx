...
export const SideNav = async({ navLinks }: { navLinks: NavItem[] }) => {
  return (
        ...
        {navLinks.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="w-max h-max absolute opacity-0 z-20 bg-sidebar invert backdrop-blur-2xl right-0 translate-x-full py-1 px-2 text-[0.6rem] rounded-l-full rounded-r-full group-hover:opacity-100 transition-all duration-700 max-sm:hidden">
                {item.title}
              </span>
            </Link>
          );
