...
import { QUEUES, FLOWS, JOBS } from "./definitions";
...
interface TranscodeFlowInput {
    videoId: string;
    inputPath: string; // S3 Key or Local Path? 
    // We assume S3 Key for input, and the worker downloads it.
    fileName: string;
}
...
export async function addTranscodeFlow(
    videoId: string, 
    inputPath: string, 
    resolutions: Array<{ width: number; height: number; bandwidth: number; name: string }>,
    metadata: { duration: number; fps: number }
) {
    console.log(`[Pipeline] 🌊 Creating Transcode Flow for ${videoId} with ${resolutions.length} resolutions`);
    // Common cleanup opts
    // Keep 100 completed jobs or 24h of history
    const JOB_OPTS = {
        removeOnComplete: { count: 100, age: 24 * 3600 },
        removeOnFail: { age: 24 * 3600 },
        attempts: 3, // Retry transient failures (e.g. S3 timeout)
        backoff: {
            type: "exponential",
            delay: 1000, // 1s, 2s, 4s
        }
